---
layout: distill
title: "Plug-and-Play: 算法的推导, 利用强化学习选择参数与证明收敛性的两种框架"
...
本文为北京大学最优化讨论班 2022 年 3 月 1 日的讲稿, 综合了

* [https://ieeexplore.ieee.org/document/8962388?arnumber=8962388](https://ieeexplore.ieee.org/document/8962388?arnumber=8962388) (IEEE Signal Processing Magzine)
* [https://proceedings.mlr.press/v119/wei20b](https://proceedings.mlr.press/v119/wei20b) (ICML 2020 Award Paper)
* [https://jmlr.org/papers/v23/20-1297.html](https://jmlr.org/papers/v23/20-1297.html) (JMLR)
* [https://epubs.siam.org/doi/abs/10.1137/20M1337168](https://epubs.siam.org/doi/abs/10.1137/20M1337168) (SIAM Journal on Imaging Sciences)
* [https://epubs.siam.org/doi/10.1137/16M1102884](https://epubs.siam.org/doi/10.1137/16M1102884) (SIAM Journal on Imaging Sciences)
...
$\mathbb{P}(y\mid x)=\mathbb{P}(\varepsilon=y-Ax)=\frac{1}{(2\pi\sigma^2)^{n/2}}\exp\left(-\frac{\|Ax-y\|^2}{2\sigma^2}\right).$

忽略常数后, 最大化后验概率在加性白高斯噪声的假设下等价于

$\min_{x}\, \frac{1}{2\sigma^2} \|Ax-y\|^2 - \ln\mathbb{P}(x).$

此时, 只要选定先验概率 $\mathbb{P}(x)$ 即可给出优化模型. 我们将 $- \ln\mathbb{P}(x)$ 记为 $\varphi(x)$, 则 $\varphi(x)$ 应当尽可能与某个函数的负对数相等, 同时使得上述优化问题易于处理. 一种常见的选择是 $\varphi(x)=\lambda\|\Psi x\|_1$, 且 $\Psi^\top\Psi=I$, $\lambda>0$. 这样的 $\varphi(x)$ 在保证优化问题凸性的同时使得输出 $\Psi x$ 较为稀疏. $\varphi(x)$ 常被称作正则项, 它向模型中添加了某种先验信息, 有大量的工作致力于选取合适的 $\varphi(x)$.
...
$\min_{x}\, \frac{1}{2\sigma^2} \|Ax-y\|^2 + \varphi(x).$

直接针对 $x$ 进行优化往往困难重重, 我们可以引入辅助变量 $v$, 将上述问题等价转化为约束优化问题

$\min_{x}\, \frac{1}{2\sigma^2} \|Ax-y\|^2 + \varphi(v), \quad \text{ s.t. } x=v.$

Alternating Direction Method of Multipliers (ADMM) 非常适合用来求解这种变量分块的问题. 引入乘子 $z$ 与罚因子 $1/\eta$, 其增广拉格朗日函数为

$\begin{aligned}
    L(x,v;u)&=\frac{1}{2\sigma^2} \|Ax-y\|^2 + \varphi(v)-z^\top(x-v) + \frac{1}{2\eta}\|x-v\|^2\\
    &=\frac{1}{2\sigma^2} \|Ax-y\|^2 + \varphi(v) + \frac{1}{2\eta}\|x-v-u\|^2-\frac{1}{2\eta}\|u\|^2.\quad(u=\eta z)
\end{aligned}$
...
&= \underset{v}{\arg\min}\, \eta\varphi(v)+\frac{1}{2}\|v-x^{k+1}+u^k\|^2\\
&=\underset{v}{\arg\min}\, \varphi(v)+\frac{1}{2\eta}\|v-x^{k+1}+u^k\|^2
\end{aligned}$
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-58978ddaaf9b9ac596e15af6453c46c3_b.jpg)  
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-995a980309e0c64d8f3108555dc81c98_b.jpg)  
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-ac731dd6f190f332d9958ca3a272423a_b.jpg)  
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-b86d9ec78fc50fd397c5f430233e1d1f_b.jpg)  
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-2004b96a934cd2606eee46fb27beaa7c_b.jpg)  
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-d825dd8c176a296d30a4956bfdf89236_b.jpg)  
...
$\rho_{\operatorname{RED}}({x})\triangleq \frac{1}{2}\langle{x}, {x}-{f}({x})\rangle, \quad\ell({x} ; {y})=\frac{1}{2 \sigma^{2}}\|{y}-{A} {x}\|_2^2.$
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-8f69e3bbb6efcd49437b7d765015da88_b.jpg)  
...
![]((20220301)Plug-and-Play_算法的推导_利用强化学习选择参数与证明收敛性的两种框架_谢中林/v2-0ddddcf9b684344934062ce819c8c491_b.jpg)  
...
$\widehat{\boldsymbol{x}}_{\mathrm{pnp}}=\underset{\boldsymbol{x}}{\operatorname{argmin}}\left\{\frac{1}{2 \sigma^{2}}\|\boldsymbol{y}-\boldsymbol{A} \boldsymbol{x}\|^{2}+\frac{1}{2 \eta} \boldsymbol{x}^{\top}\left(\boldsymbol{W}^{-1}-\boldsymbol{I}\right) \boldsymbol{x}\right\}.$

而 RED 则试图求解

$\widehat{\boldsymbol{x}}_{\mathrm{red}}=\underset{\boldsymbol{x}}{\operatorname{argmin}}\left\{\frac{1}{2 \sigma^{2}}\|\boldsymbol{y}-\boldsymbol{A x}\|^{2}+\frac{1}{2 \eta} \boldsymbol{x}^{\top}(\boldsymbol{I}-\boldsymbol{W}) \boldsymbol{x}\right\}.$
...
$\widehat{\boldsymbol{x}}=\underset{\boldsymbol{x}}{\operatorname{argmin}}\;\frac{1}{2 \sigma^{2}}\|\boldsymbol{y}-\boldsymbol{A x}\|^{2}-\ln \tilde{p}(\boldsymbol{x} ; \eta).$
...
$\min_{\theta}\;\mathbb{E}\|\boldsymbol{x}-\boldsymbol{f}_{\theta}(\boldsymbol{z})\|^2,\quad\text{where}\quad \boldsymbol{x}\sim \widehat{p},\quad \boldsymbol{z}=\boldsymbol{x}+\mathcal{N}(\mathbf{0}, \eta \boldsymbol{I}).$

利用最小均方误差 (MMSE) 所具备的正交性质 (类似于欧氏空间中的正交分解), 损失函数又可以分解为

$\mathbb{E}\left\|\boldsymbol{x}-\boldsymbol{f}_{\boldsymbol{\theta}}(\boldsymbol{z})\right\|^{2}= \mathbb{E}\left\|\boldsymbol{x}-\boldsymbol{f}_{\mathrm{mmse}}(z ; \eta)\right\|^{2} +\mathbb{E}\left\|\boldsymbol{f}_{\mathrm{mmse}}(\boldsymbol{z} ; \eta)-\boldsymbol{f}_{\boldsymbol{\theta}}(\boldsymbol{z})\right\|^{2}.$

再次使用 Tweedie's formula, 我们得到

$\begin{aligned}
\widehat{\boldsymbol{\theta}} 
&=\underset{\boldsymbol{\theta}}{\operatorname{argmin}}\;\mathbb{E}\|\boldsymbol{x}-\boldsymbol{f}_{\theta}(\boldsymbol{z})\|^2\\
&=\underset{\boldsymbol{\theta}}{\operatorname{argmin}}\; \mathbb{E}\left\|\boldsymbol{f}_{\mathrm{mmse}}(z ; \eta)-\boldsymbol{f}_{\boldsymbol{\theta}}(z)\right\|^{2} \\
&=\underset{\boldsymbol{\theta}}{\operatorname{argmin}}\; \mathbb{E}\|\nabla \ln \tilde{p}(z ; \eta)-\frac{1}{\eta}\left(f_{\boldsymbol{\theta}}(z)-z\right)\|^{2}.
\end{aligned}$
...
$\begin{aligned}
\boldsymbol{h}(\boldsymbol{z} ; \eta) & \triangleq \underset{\boldsymbol{x}\in \mathbb{R}^{n}}{\arg\min}\;\frac{1}{2 \sigma^{2}}\|\boldsymbol{y}-\boldsymbol{A} \boldsymbol{x}\|^{2}+\frac{1}{2 \eta}\|\boldsymbol{x}-\boldsymbol{z}\|^{2} \\
...
> 
> $\|T(\boldsymbol{x})-\boldsymbol{z}\|^{2} \leq\|\boldsymbol{x}-\boldsymbol{z}\|^{2}+d\|T(\boldsymbol{x})-\boldsymbol{x}\|^{2},$
> 
> or equivalently
> 
> $\frac{1-d}{2}\|\boldsymbol{x}-T(\boldsymbol{x})\|^{2} \leq\langle\boldsymbol{x}-T(\boldsymbol{x}), \boldsymbol{x}-\boldsymbol{z}\rangle.$
